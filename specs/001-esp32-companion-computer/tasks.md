...
- [ ] T002 Verify OTA partition layout: confirm `partitions.csv` has two OTA app slots (ota_0, ota_1) ≥ 6 MB each; search the codebase for any existing call to `esp_ota_mark_app_valid_cancel_rollback()` — if found, **remove it** before proceeding; T023 is the single authoritative call site and any pre-existing call would mark the current (potentially bad) firmware as valid, silently defeating OTA rollback protection
...
- [ ] T010 [P] Rename `src/mavesp8266_httpd.h` → `src/httpd.h` and `.cpp` → `httpd.cpp`: update header guard; rename class `MavESP8266Httpd` → `Httpd`; do NOT add `ota_state_t` here — it is declared in `src/ota.h` (created in T016); update all includes
