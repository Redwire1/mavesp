...
- [ ] T001 Verify current build: run `pio run -e esp32s3-cam` on unmodified codebase; note any existing warnings to distinguish from regressions introduced in Phase 2
- [ ] T002 Verify OTA partition layout: confirm `partitions.csv` has two OTA app slots (ota_0, ota_1) ≥ 6 MB each and that `esp_ota_mark_app_valid_cancel_rollback()` is NOT already called (if it is, no rollback safety net exists — document finding)
- [ ] T003 [P] Research and pin `ESPAsyncWebServer` + `AsyncTCP` library versions compatible with `espressif32` platform version in use; add pinned `lib_deps` entries to `platformio.ini` `[env:esp32s3-cam]` (needed in Phase 4 — pin now to avoid surprise)
- [ ] T004 [P] Research and pin `Micro-RTSP` by geeksville to a known-good commit/tag compatible with ESP32 Arduino core 2.x; add pinned entry to `platformio.ini` `lib_deps` (needed in Phase 4)
...
- [ ] T005 Rename `src/mavesp8266.h` → `src/bridge.h` and `src/mavesp8266.cpp` → `src/bridge.cpp`: update header guard `MAVESP8266_H` → `BRIDGE_H`; rename class `MavESP8266Bridge` → `Bridge`; rename `MavESP8266World` → `World`; rename `MavESP8266Log` → `Log`; rename `MavESP8266Update` → `Update` (or `OtaUpdate` to avoid collision with Arduino Update class — choose `OtaUpdate`); rename `MavESP8266UpdateImp` in `main.cpp` → `OtaUpdateImp`; rename `linkStatus` struct → `link_status_t`; rename `MAVESP8266_VERSION_*` → `VERSION_*`; add `bool is_armed` and `uint32_t last_heartbeat_ms` fields to `link_status_t` (used by OTA gate in US1 before `telemetry.h` exists)
- [ ] T006 [P] Rename `src/mavesp8266_component.h` → `src/component.h` and `.cpp` → `component.cpp`: update header guard; rename class `MavESP8266Component` → `Component`; update `#include "mavesp8266.h"` → `#include "bridge.h"` and all other old includes
- [ ] T007 [P] Rename `src/mavesp8266_gcs.h` → `src/gcs.h` and `.cpp` → `gcs.cpp`: update header guard; rename class `MavESP8266GCS` → `Gcs`; update all includes; ensure `gcs_client_t` struct is declared in `gcs.h` per data-model.md
- [ ] T008 [P] Rename `src/mavesp8266_vehicle.h` → `src/vehicle.h` and `.cpp` → `vehicle.cpp`: update header guard; rename class `MavESP8266Vehicle` → `Vehicle`; update all includes
- [ ] T009 [P] Rename `src/mavesp8266_parameters.h` → `src/parameters.h` and `.cpp` → `parameters.cpp`: update header guard; rename class `MavESP8266Parameters` → `Parameters`; rename struct `stMavEspParameters` → `parameters_t`; rename `MAVESP_WIFI_MODE_*` → `WIFI_MODE_*`; update all includes
- [ ] T010 [P] Rename `src/mavesp8266_httpd.h` → `src/httpd.h` and `.cpp` → `httpd.cpp`: update header guard; rename class `MavESP8266Httpd` → `Httpd`; add `ota_state_t` enum declaration per data-model.md; update all includes
- [ ] T011 [P] Rename `src/mavesp8266_ppm.h` → `src/ppm.h` and `.cpp` → `ppm.cpp`: update header guard; rename class `MavESP8266PPM` → `Ppm`; update all includes
- [ ] T012 Update `src/main.cpp`: replace all 7 `#include "mavesp8266*.h"` directives with renamed equivalents (`bridge.h`, `component.h`, `gcs.h`, `vehicle.h`, `httpd.h`, `parameters.h`, `ppm.h`); rename `MavESP8266UpdateImp` → `OtaUpdateImp`; update all class name references throughout the file
- [ ] T013 Update `src/bridge.h` forward declarations: replace `class MavESP8266Parameters` etc. with `class Parameters`, `class Component`, `class Vehicle`, `class Gcs`, `class Ppm`; verify all cross-include references in bridge.h, component.h, gcs.h, vehicle.h are consistent after renames
- [ ] T014 Build verification: `pio run -e esp32s3-cam` MUST produce 0 errors, 0 warnings; resolve any warnings before proceeding
- [ ] T015 Physical flash + smoke test: `pio run -e esp32s3-cam --target upload`; verify device boots, WiFi AP appears, MAVLink forwarding works with a GCS client
...
- [ ] T016 [P] [US1] Create `src/ota.h`: declare `otaBegin()` → `bool`, `otaWrite(uint8_t* data, size_t len)` → `size_t`, `otaEnd()` → `bool`; include `ota_state_t` enum (OTA_IDLE, OTA_IN_PROGRESS, OTA_COMPLETE, OTA_ERROR); declare `otaGetState()` → `ota_state_t`
- [ ] T017 [US1] Implement `src/ota.cpp`: `otaBegin()` reads `getWorld()->getVehicle()->getLinkStatus().is_armed` — returns false and sets state to OTA_ERROR if true; else calls `Update.begin(UPDATE_SIZE_UNKNOWN)`, sets state OTA_IN_PROGRESS; `otaWrite()` calls `Update.write()`; `otaEnd()` calls `Update.end()`, on success calls `esp_ota_mark_app_valid_cancel_rollback()` and sets state OTA_COMPLETE (depends on T016)
...
- [ ] T022 [P] [US1] Update `src/parameters.cpp`: register the 9 new parameters from data-model.md: `CAM_RESOLUTION` (UINT8, default 6), `CAM_QUALITY` (UINT8, default 10), `PWM_ENABLED` (UINT8, default 0), `PWM_SERVO_CHAN` (UINT8, default 5), `PWM_GPIO` (UINT8, default 14), `LLM_API_ENABLED` (UINT8, default 0), `LLM_API_SECRET` (CHAR[64], default ""), `LLM_PUSH_URL` (CHAR[128], default ""), `LLM_THRESH_BATT` (UINT8, default 20)
...
- [ ] T028 [US2] Implement `src/camera.cpp` `cameraInit()`: populate `esp_camera_config_t` using pin constants from `variants/esp32s3-cam/pins_arduino.h` and `camera_config_t` framesize/quality params; call `esp_camera_init()`; call `esp_camera_sensor_get()` to apply quality setting; return false if init fails (camera absent); set `camera_config_t.enabled = false` on failure (depends on T026, T027)
- [ ] T029 [US2] Implement `src/camera.cpp` `cameraStreamTask()`: FreeRTOS task, Core 0, priority 3; loop: `esp_camera_fb_get()` (PSRAM frame buffer via `heap_caps_malloc(MALLOC_CAP_SPIRAM)`); if RTSP session active push frame to `CStreamer`; if HTTP sink registered write frame to `AsyncResponseStream`; `esp_camera_fb_return()`; `taskYIELD()` to prevent starving Core 0; exit cleanly when task deleted (depends on T028)
...
- [ ] T031 [US2] Migrate `src/httpd.cpp` from `WebServer` to `AsyncWebServer`: replace `WebServer _webServer` instance with `AsyncWebServer _webServer(80)`; rewrite GET `/`, `/status`, `/getstatus`, `/getParameters`, `/setParameters` handlers as `_webServer.on(path, HTTP_GET, [](AsyncWebServerRequest* request){...})`; remove all `_webServer.handleClient()` calls from the update loop
...
- [ ] T039 [US3] Implement `src/telemetry.cpp` `telemetryGetJson()`: write to static `char _json_buf[512]`; format JSON matching `contracts/http-api.md` `/api/v1/telemetry` response exactly (field names: `armed`, `battery_voltage_mv`, `battery_remaining_pct`, `gps_fix_type`, `gps_satellites`, `lat`, `lon`, `alt_mm`, `heading_deg`, `groundspeed_ms`, `system_status`, `link_quality` sub-object); no heap allocation; return pointer to `_json_buf` (depends on T038)
...
- [ ] T044 [US3] Update `src/httpd.cpp`: add `LittleFS.begin()` in `Httpd::begin()`; add `GET /status` → serve `data/index.html` from LittleFS (`request->send(LittleFS, "/index.html", "text/html")`); add `GET /params` → serve `data/params.html`; add catch-all static file handler for `data/` assets with correct MIME types (depends on T042, T043)
- [ ] T045 [US3] Build firmware + OTA push + upload LittleFS assets: `pio run -e esp32s3-cam`; OTA push firmware; `pio run -e esp32s3-cam --target uploadfs` (LittleFS upload requires USB or a separate LittleFS OTA mechanism — document which method is used)
...
- [ ] T051 [US4] Build + OTA push Increment 4; HIL test: (a) channel 5 at 1500 µs → 50% duty at 490 Hz; (b) channel 5 at 1000 µs → 0% duty; (c) UART disconnected > 1 s → duty falls to 0%; (d) `PWM_ENABLED = 0` → no LEDC init, GPIO14 remains hi-z; (e) MAVLink forwarding and camera stream unaffected while PWM active

**Checkpoint**: Motor driver functional; failsafe verified; MAVLink bridge unaffected.

---

## Phase 7: User Story 5 — LLM Companion API (Priority: P5)

**Goal**: Authenticated REST API over WiFi for MimiClaw LLM companion device.
Read telemetry, issue allowlisted MAVLink commands, receive proactive event push
notifications. Fully gated by `LLM_API_ENABLED` parameter.

**Independent Test**: `curl` or Postman from a WiFi client; verify all response
codes, authentication, command dispatch, and event push. No MimiClaw device
required for testing.

- [ ] T052 [P] [US5] Create `src/llm_api.h`: declare `llm_cmd_type_t` enum (LLM_CMD_SET_PARAMETER, LLM_CMD_REQUEST_MODE), `llm_cmd_t` struct, `llm_event_type_t` enum (LLM_EVENT_BATTERY_LOW, LLM_EVENT_LINK_LOST, LLM_EVENT_LINK_RESTORED), `llm_event_t` struct — all exactly per `data-model.md`; declare `void llmApiRegisterHandlers(AsyncWebServer* server)`; declare `void llmPushEventTask(void* arg)`
- [ ] T053 [US5] Implement `src/llm_api.cpp` `llmApiRegisterHandlers()`: if `LLM_API_ENABLED == 0`, register `server->on("/api/v1/*", HTTP_ANY, [](r){ r->send(404); })` catch-all and return; else register full handlers for GET `/api/v1/telemetry` and POST `/api/v1/command` (depends on T052)
- [ ] T054 [US5] Implement GET `/api/v1/telemetry` handler: validate `X-Api-Key` header using `memcmp()` against `LLM_API_SECRET` parameter (constant-time — compare full 64-byte buffer regardless of mismatch position to avoid timing side-channel); if mismatch: log rejection to `link_status_t.gcs_parse_errors` (reuse as auth-failure counter), return 401; if match: return `telemetryGetJson()` with `Content-Type: application/json` (depends on T039, T053)
- [ ] T055 [US5] Implement POST `/api/v1/command` handler: validate `X-Api-Key` (same constant-time check); parse JSON body (`{"type":"SET_PARAMETER","param_id":"...","param_value":...}` or `{"type":"REQUEST_MODE","mode_value":...}`); populate `llm_cmd_t`; validate `type` against allowlist → 400 if unknown; for SET_PARAMETER: find matching `parameters_t` entry by `id` → 422 if not found; check `!read_only` → 422 if read_only; validate value range → 422 if out of range; dispatch `MAVLINK_MSG_ID_PARAM_SET` to autopilot via Vehicle; return 202; for REQUEST_MODE: dispatch `MAV_CMD_DO_SET_MODE` via `COMMAND_LONG`; return 202 (depends on T052, T053)
- [ ] T056 [US5] Implement `src/llm_api.cpp` `llmPushEventTask()`: FreeRTOS task, Core 0, priority 2; static `llm_event_t _event_buf`; static debounce flags `_batt_low_active`, `_link_lost_active`; every 5 s: check `telemetry_state_t.battery_remaining < LLM_THRESH_BATT` — if crossed and `!_batt_low_active`, build LLM_EVENT_BATTERY_LOW event, set `_batt_low_active = true`; check if `millis() - last_heartbeat_ms > 10000` — handle LLM_EVENT_LINK_LOST/LINK_RESTORED with `_link_lost_active` flag; push via `HTTPClient` with 3 s timeout (`client.setTimeout(3000)`); log push failure to Serial but do NOT block or retry — task continues regardless; use `LLM_PUSH_URL` from Parameters (depends on T052, T039)
- [ ] T057 [US5] Update `src/main.cpp`: `#include "llm_api.h"`; after `AsyncWebServer` init call `llmApiRegisterHandlers(&_webServer)`; if `LLM_API_ENABLED == 1`: `xTaskCreatePinnedToCore(llmPushEventTask, "llmpush", 4096, NULL, 2, NULL, 0)` on Core 0 (depends on T053, T056)
- [ ] T058 [US5] Build + OTA push Increment 5
- [ ] T059 [US5] HIL test: (a) `GET /api/v1/telemetry` with correct key → 200 JSON with expected fields; (b) wrong key → 401; (c) `POST /api/v1/command` `SET_PARAMETER UART_BAUDRATE 115200` → 202, `/getParameters` confirms new value; (d) unknown type `RAW_MAVLINK` → 400; (e) read-only param → 422; (f) set `LLM_THRESH_BATT` to 99 to trigger battery event → push fires once to `LLM_PUSH_URL`; (g) `LLM_API_ENABLED = 0` → all `/api/v1/` return 404; (h) heap > 20 KB while all features active

**Checkpoint**: Full LLM companion API operational; security controls verified.
...
- [ ] T063 Full performance validation with all features active (US1–US5): measure MAVLink end-to-end latency with Wireshark (must be < 10 ms); measure free heap with `/getstatus` (must be > 20 KB); measure firmware binary size from PlatformIO build output (must be < 1.5 MB); document pass/fail results in `quickstart.md` Performance section
...
- **Phase 5 (US3)**: Depends on Phase 4 (needs AsyncWebServer from US2; `telemetryGetJson()` referenced by US5)
- **Phase 6 (US4)**: Depends on Phase 3 — can run in parallel with Phase 4 and 5 if capacity allows
- **Phase 7 (US5)**: Depends on Phase 4 (AsyncWebServer) and Phase 5 (`telemetryGetJson()`)
...
Foundation (Phase 2)
  └── US1: MAVLink Bridge + OTA (Phase 3) ← first physical flash required here
        └── US2: Camera Streaming (Phase 4)
              └── US5: LLM API (Phase 7) ← also depends on US3
        └── US3: Web Dashboard (Phase 5) ──┘
        └── US4: Motor Driver (Phase 6) ← independent of US2/US3/US5
```
...
**Phase 7**: T052 first; T053–T056 sequential; T057 last before build
...
| 5 | 6 | Motor driver with failsafe |
| 6 | 7 + 8 | LLM API + polish |
